...
const contradictions = {
  smm_contradictions:  safe_arr(safe_get(smm, 'contradictions')).slice(0, 5),
  ...
  items: [
    ...safe_arr(safe_get(smm,  'contradictions')).map(c => ({
      ...
      severity: (c && typeof c === 'object' && c.severity) || 'medium',
    })),
    ...safe_arr(safe_get(comp, 'contradictions')).map(c => ({
      agent: 'Конкуренти', type: 'contradiction',
      text: typeof c === 'string' ? c : (c.text || c.issue || c.contradiction || ''),
      severity: (c && typeof c === 'object' && c.severity) || 'medium',
    })),
    ...safe_arr(safe_get(smm,  'key_risks') || safe_get(smm,  'key_risk')).map(r => ({
      agent: 'SMM', type: 'risk',
      text: typeof r === 'string' ? r : (r.text || r.risk || r.description || ''),
      severity: (r && typeof r === 'object' && r.severity) || 'high',
    })),
    ...safe_arr(safe_get(comp, 'key_risks') || safe_get(comp, 'key_risk')).map(r => ({
      agent: 'Конкуренти', type: 'risk',
      text: typeof r === 'string' ? r : (r.text || r.risk || r.description || ''),
      severity: (r && typeof r === 'object' && r.severity) || 'high',
    })),
  ].filter(item => item.text && item.text.length > 3).slice(0, 8),
};
